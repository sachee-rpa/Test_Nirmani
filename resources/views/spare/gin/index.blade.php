            ...
            <table style="width: 100%;" id="" class="table table-hover table-striped table-bordered">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $object->customer_po->name ?? '' }}</td>
                        <td>{{ $object->date ?? '' }}</td>
                        <td>{{ $object->customer_type_no ?? '' }}</td>
                        <td>{{ $object->invoice_type_no ?? '' }}</td>
                        <td>{{ $object->remarks ?? '' }}</td>
                        <td>
                        
                            <a href="{{ route($route_name .'print', $object)   }}" target="_blank"  class="btn  btn-icon btn-primary btn-sm"
                                href="#" role="button">
...
@if ($search ?? '')
@section('search')
@includeWhen($search ?? '', 'spare.grn.search')
@endsection
@endif